...
stacks. Such protocol stacks can be useful for various initiatives that work on
self-star properties in computing by providing an algorithm that configures and
...
\item Execution of the PPE in ksoftirqd context. This approach achieves
      approx. $95\%$ of the performance of the default stack.
\end{compactitem}
...
In the short-term, we will compare the performance of real scenarios in LANA with the performance in other systems (e.g., default Linux stack, Click router, etc.). 
...
In the long-term, we envisage a system that requires less configuration as compared to today's networks and that is able to adapt itself to changing network conditions.   
