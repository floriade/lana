...
\frame {
	...
	\arrayrulecolor{gray}
		...
		\hline\\[-2.1ex]
		\textit{Threads\footnotemark[0]}     & 1 FB $\hat{=}$ 1 Thread     & "1 CPU $\hat{=}$ 1 Thread"\\[1pt]
		\hline\\[-2.1ex]
		\textit{Centrality\footnotemark[0]}  & Central MINMEX        & Cache-friendly, decentralized\\[1pt]
		\hline\\[-2.1ex]
		\textit{Address space}               & User- and Kernelspace & Kernelspace\\
	\end{tabular}
	\footnotetext{under the assumption that multiple CPUs are used}
}
...
\frame {
	...
	\begin{itemize}
		...
		\item Is being extended with \textbf{trafgen}, a packet generator\medskip
		\begin{itemize}
			\item Used for performance evaluation, stress-testing, \newline debugging, and the development of LANA\medskip
			\item Uses high-performance Zero-Copy Linux kernel API\medskip
			\item First evaluation shows maximum 422.000 pps (fig. 2, \newline 64 Byte, Gigabit-Ethernet, 2.6 Ghz machine)\medskip
		\end{itemize}
		\item \textbf{ifpps} used to gather kernel networking statistics
	\end{itemize}
}
